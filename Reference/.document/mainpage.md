...
| 作者情報表示API | Full | なし |
| スコアボードAPI | Full | なし |
| スクリーンショットAPI | Full | RPGアツマールから直接スクリーンショットを取る場合は2回押して貰う必要があります |
...
## スクリーンショットのデフォルト画像を変更しよう

"RPGアツマールAPI for Unity"のデフォルトスクリーンショット画像が気に入りませんか？  
ご安心下さい、デフォルトスクリーンショット画像は変更することが可能です。  
ゲーム側の雰囲気に合わせた、デフォルトスクリーンショット画像を設定することで  
プレイヤーへのメッセージを入れたり"RPGアツマールAPI for Unity"感を抑えることが出来ます。  
@subpage screenshotdefault に詳しい実装方法が記載されています。
